...
from .models import *
from .ml.predictor import run_ml_pipeline
...
# ─────────────────────────────────────────────────────────
# ForgotPassword Step 2 helper: Verify OTP ONLY (no register)
# POST /vvit/verify-otp-only/
# Body: { email, otp }
# ─────────────────────────────────────────────────────────
class VerifyOTPOnlyView(APIView):
    def post(self, request):
        ...
        if not cached_otp:
            return Response({'error': 'OTP expired or not sent. Please request a new OTP.'}, status=400)
        if str(cached_otp) != otp:
            return Response({'error': 'Invalid OTP. Please check and try again.'}, status=400)

        # Mark OTP as verified (keep for 10 mins for password reset step)
        cache.set(f'otp_verified_{email}', True, timeout=600)
        # Delete the OTP itself so it can't be reused
...
# ─────────────────────────────────────────────────────────
# ForgotPassword Step 3: Reset Password
# POST /vvit/reset-password/
...
class ResetPasswordView(APIView):
    def post(self, request):
        ...
        try:
            institution = Institution.objects.get(email__iexact=email)
        except Institution.DoesNotExist:
            return Response({'error': 'No institution found with this email.'}, status=404)

        institution.password = make_password(new_password)
        institution.save()
...
class SendOTPView2(APIView):
    def post(self, request):
        ...
        try:
            send_mail(
                subject='AICTE Portal — Email Verification OTP',
                message=f'''Dear Applicant,

Your OTP for AICTE Institution Registration is:
...
Ministry of Education, Government of India''',
        ...
        return Response({'message': f'OTP sent to {email}', 'email': email})
...
class VerifyOTPAndRegisterView(APIView):
    def post(self, request):
        ...
        # Now register the institution (same logic as RegisterView)
        from .models import Institution
        ...
        if not name or not password:
            return Response({'error': 'Institution name and password required'}, status=400)
        ...
        inst = Institution.objects.create(
            ...
            year_established=int(data.get('year_established') or 2000),
...
class VvitUploadPDFToS3View(APIView):
    def post(self, request, format=None):
        institution_id = request.POST.get('institution_id')
        section_type   = request.POST.get('section_type', '').strip().lower()
        pdf_file       = request.FILES.get('pdf_file')
        if not institution_id or not section_type or not pdf_file:
            return Response({'error': 'institution_id, section_type and pdf_file required.'}, status=400)
        try:
            inst = Institution.objects.get(pk=institution_id)
        except Institution.DoesNotExist:
            return Response({'error': 'Institution not found.'}, status=404)
        safe_name = inst.institution_name.replace(' ', '_').lower()[:25]
        s3_key = f"aicte/inst_{institution_id}_{safe_name}/{section_type}/{section_type}_2024-25.pdf"
        s3_url = f"https://{settings.AWS_STORAGE_BUCKET_NAME}.s3.ap-south-1.amazonaws.com/{s3_key}"
        try:
            s3 = _s3_client()
            pdf_file.seek(0)
            s3.upload_fileobj(pdf_file, BUCKET, s3_key, ExtraArgs={'ContentType': 'application/pdf'})
        except ClientError as e:
            return Response({'error': f'S3 upload failed: {str(e)}'}, status=500)
        DisclosureSection.objects.filter(
            institution_id=institution_id,
            section_type=section_type,
        ).update(s3_url=s3_url, s3_key=s3_key)
        return Response({'s3_url': s3_url, 's3_key': s3_key}, status=200)

class VvitGetInstitutionPDFsView(APIView):
    def get(self, request, format=None):
        institution_id = request.GET.get('institution_id')
        if not institution_id:
            return Response({'error': 'institution_id required'}, status=400)
        sections = DisclosureSection.objects.filter(institution_id=institution_id).order_by('section_type')
        s3 = _s3_client()
        out = []
        for sec in sections:
            presigned = None
            if getattr(sec, 's3_key', None):
                try:
                    presigned = s3.generate_presigned_url(
                        'get_object',
                        Params={'Bucket': BUCKET, 'Key': sec.s3_key},
                        ExpiresIn=3600,
                    )
                except ClientError:
                    presigned = getattr(sec, 's3_url', '') or ''
            out.append({
                'section_type':  sec.section_type,
                's3_url':        getattr(sec, 's3_url', '') or '',
                'presigned_url': presigned or getattr(sec, 's3_url', '') or '',
                'uploaded_at':   sec.uploaded_at.strftime('%d %b %Y, %I:%M %p'),
                'status':        sec.status,
...
class UploadDisclosureView(View):
    ...
    def post(self, request):
        ...
        # Compute fresh risk
        risk_result = run_ml_pipeline(inst) or {}
...
class DashboardView(View):
    def get(self, request):
        ...
        ml_result = run_ml_pipeline(inst) if latest_risk else {}
        return JsonResponse({
            'institution_id':    inst.id,
            'institution_name':  inst.institution_name,
            'aicte_id':          inst.aicte_id,
            ...
            'approval_probability': ml_result.get('approval_probability', 0),            
            'latest_approval': {
...
class AIRiskView(View):
    def get(self, request):
        ...
        ml_result = run_ml_pipeline(inst)        
        return JsonResponse({
